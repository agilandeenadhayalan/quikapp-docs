...
QuckChat maintains three UAT environments for different testing purposes:

| Environment | URL | Purpose |
|-------------|-----|---------|
| **UAT1** | `https://uat1.quckchat.com` | Feature testing (Sprint A) |
| **UAT2** | `https://uat2.quckchat.com` | Feature testing (Sprint B) |
| **UAT3** | `https://uat3.quckchat.com` | Regression testing |
...
docker build -t registry.quckchat.dev/backend:$UAT_ENV-$VERSION ./backend
docker push registry.quckchat.dev/backend:$UAT_ENV-$VERSION

# Tag as latest for the UAT environment
docker tag registry.quckchat.dev/backend:$UAT_ENV-$VERSION \
           registry.quckchat.dev/backend:$UAT_ENV-latest
docker push registry.quckchat.dev/backend:$UAT_ENV-latest

# Update kubeconfig
aws eks update-kubeconfig --name quckchat-uat --region us-east-1

# Deploy
kubectl apply -k k8s/overlays/$UAT_ENV
kubectl rollout status deployment/backend -n quckchat-$UAT_ENV --timeout=300s

echo "Deployment to $UAT_ENV complete!"
echo "URL: https://$UAT_ENV.quckchat.com"
```
...
# Scale down services
kubectl scale deployment --all --replicas=0 -n quckchat-$UAT_ENV

# Restore from sanitized production snapshot
aws rds restore-db-cluster-from-snapshot \
  --db-cluster-identifier quckchat-$UAT_ENV-new \
...
kubectl create secret generic db-credentials \
  --from-literal=host=quckchat-$UAT_ENV-new.cluster-xxxxx.rds.amazonaws.com \
  --dry-run=client -o yaml | kubectl apply -f - -n quckchat-$UAT_ENV

# Scale up services
kubectl scale deployment --all --replicas=2 -n quckchat-$UAT_ENV
...
def sanitize_user(user):
    ...
    return {
        **user,
        'email': f"uat.user.{email_hash}@quckchat-test.com",
...
def sanitize_file(file):
    ...
    return {
        **file,
        'url': f"https://cdn.uat.quckchat.com/placeholder/{file['type']}",
...
| Email | Password | Role |
|-------|----------|------|
| uat1.admin@quckchat.com | Uat1Admin123! | Super Admin |
| uat1.owner@quckchat.com | Uat1Owner123! | Workspace Owner |
| uat1.member@quckchat.com | Uat1Member123! | Member |
| uat1.guest@quckchat.com | Uat1Guest123! | Guest |

### UAT2 Users

| Email | Password | Role |
|-------|----------|------|
| uat2.admin@quckchat.com | Uat2Admin123! | Super Admin |
| uat2.owner@quckchat.com | Uat2Owner123! | Workspace Owner |
| uat2.member@quckchat.com | Uat2Member123! | Member |
| uat2.guest@quckchat.com | Uat2Guest123! | Guest |

### UAT3 Users

| Email | Password | Role |
|-------|----------|------|
| uat3.admin@quckchat.com | Uat3Admin123! | Super Admin |
| uat3.owner@quckchat.com | Uat3Owner123! | Workspace Owner |
| uat3.member@quckchat.com | Uat3Member123! | Member |
| uat3.guest@quckchat.com | Uat3Guest123! | Guest |
...
| Tool | UAT1 | UAT2 | UAT3 |
|------|------|------|------|
| Grafana | [uat1.grafana](https://grafana.uat1.quckchat.com) | [uat2.grafana](https://grafana.uat2.quckchat.com) | [uat3.grafana](https://grafana.uat3.quckchat.com) |
| Logs | [uat1.logs](https://logs.uat1.quckchat.com) | [uat2.logs](https://logs.uat2.quckchat.com) | [uat3.logs](https://logs.uat3.quckchat.com) |
...
openvpn --config quckchat-uat.ovpn
```
...
kubectl get pods -n quckchat-uat1
kubectl get pods -n quckchat-uat2
kubectl get pods -n quckchat-uat3
```
...
stakeholders:
  - name: "Product Owner"
    email: "po@quckchat.com"
    approved: true
    date: "2024-01-18"
  - name: "Business Analyst"
    email: "ba@quckchat.com"
    approved: true
    date: "2024-01-19"
