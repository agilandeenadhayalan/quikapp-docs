...
The local environment enables developers to run the entire QuckChat stack on their machine with mocked external services and seeded data.
...
services:
  ...
  mongodb:
    image: mongo:6.0
    environment:
      MONGO_INITDB_ROOT_USERNAME: quckchat
...
CDN_URL=http://localhost:4566/quckchat-local
...
MYSQL_USER=quckchat
MYSQL_PASSWORD=localpass
MYSQL_DATABASE=quckchat_auth

# Database - MongoDB
MONGODB_URI=mongodb://quckchat:localpass@localhost:27017/quckchat?authSource=admin
...
# Seed PostgreSQL
psql -h localhost -U quckchat -d quckchat -f ./seeds/postgres/workspaces.sql
psql -h localhost -U quckchat -d quckchat -f ./seeds/postgres/channels.sql

# Seed MySQL
mysql -h localhost -u quckchat -plocalpass quckchat_auth < ./seeds/mysql/users.sql
mysql -h localhost -u quckchat -plocalpass quckchat_auth < ./seeds/mysql/roles.sql

# Seed MongoDB
mongosh "mongodb://quckchat:localpass@localhost:27017/quckchat?authSource=admin" ./seeds/mongodb/messages.js

# Seed Elasticsearch indices
curl -X PUT "localhost:9200/quckchat-messages" -H "Content-Type: application/json" -d @./seeds/elasticsearch/messages-mapping.json
curl -X POST "localhost:9200/quckchat-messages/_bulk" -H "Content-Type: application/json" --data-binary @./seeds/elasticsearch/messages-data.json

# Setup LocalStack S3
aws --endpoint-url=http://localhost:4566 s3 mb s3://quckchat-local
aws --endpoint-url=http://localhost:4566 s3 cp ./seeds/files/ s3://quckchat-local/files/ --recursive

# Setup Vault secrets
export VAULT_ADDR=http://localhost:8200
export VAULT_TOKEN=local-dev-token
vault kv put secret/database/postgres host=localhost port=5432 username=quckchat password=localpass
